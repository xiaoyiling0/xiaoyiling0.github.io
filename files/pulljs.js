...
var result = function(){
	...
	nb.setAttribute('onclick','settlement()');
	nn=ne.appendChild(document.createElement('span'));		//名字及名字背景出现	
	nn.appendChild(document.createTextNode(namearray[0]));
	nn.setAttribute('id','mainname');
	nn.style.color = namecolour[0];
	nn.setAttribute('onclick','settlement()');
};

var settlement=function(){
	if(x<n){
		nn.innerText = namearray[x];
		nn.style.color = namecolour[x];
		x++;
	}else{
